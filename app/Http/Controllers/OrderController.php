	...
	public function getOrder()
	{
		...
		->addColumn('action', function($order){
			...
			<form action="'.route("order.deleteCart",$order->code).'" method="post">
				<input class="btn btn-danger" value="Xóa" type="submit" style="display: inline-block" />
				'.csrf_field().'
				</form>';
		})
		->editColumn('status',function($order){
			if($order->status == 0){
				return '<a class="btn btn-primary btn-detail" data-id="'.$order->code.'"​ href="'.route('order.checkbill',$order->code).'">Chưa xác nhận</a>';
				// return "dat";
			}
			if($order->status == 1){
				return '<a class="btn btn-success btn-detail" data-id="'.$order->code.'"​ href="'.route('order.checkbill',$order->code).'">Đang giao hàng</a>';
			}
