...
class ShopController extends Controller
{
    ...
    public function infoUser(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $order->total = priceToFloat(Cart::subtotal());
            $order->status = 0; 
            $order->customer_id = 0; 
            $order->save();
            ...
            foreach (Cart::content() as $value) {
                ...
                $detail['sub_total'] = (($value->price)*($value->qty));
                $detail->save();
            }
            //send email
                $email =  $order->email;
                // $input = $request->all();
                Mail::send('form', array('data'=>Cart::content(), 'order'=> $order), function($message) use ($email){
                            $message->to($email, 'ShopCHT')->subject('Đặt hàng thành công !');
                        });
                // Session::flash('flash_message', 'Send message successfully!');
